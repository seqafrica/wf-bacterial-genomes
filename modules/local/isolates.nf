...
process resfinder_hamronize {
    ...
    input:
        tuple val(meta), path("${meta.alias}_resfinder_results")
    output:
        tuple val(meta), path("${meta.alias}_hamro_resfinder.tsv")
    script:
    """
    # The sed is to make the hamronize-generated isolate name match with the other tools (meta.alias)
    hamronize resfinder ${meta.alias}_resfinder_results/${meta.alias}_resfinder.json | sed -Ee 's/input_genome/${meta.alias}/' >${meta.alias}_hamro_resfinder.tsv
    """
}
...
process amrfinder {
    ...
    """
    gunzip -c input_genome.fasta.gz > input_genome.fasta
    amrfinder -n input_genome.fasta -o ${meta.alias}_amrfinder_results.tsv
    """
}
...
process amrfinder_hamronize {
    ...
    input:
        tuple val(meta), path("${meta.alias}_amrfinder_results.tsv")
    output:
        tuple val (meta), path("${meta.alias}_hamro_amrfinder.tsv")
    script:
    """
    hamronize amrfinderplus --reference_database_version 6.0.3 --analysis_software_version 4.0.3 --input_file_name ${meta.alias} --output ${meta.alias}_hamro_amrfinder.tsv ${meta.alias}_amrfinder_results.tsv
    """
}
...
process rgi {
    ...
    script:
    """
    rgi main -i input_genome.fasta.gz -o ${meta.alias}_rgi_results
    """
}
...
process rgi_hamronize {
    ...
    input:
        tuple val(meta), path("${meta.alias}_rgi_results.txt")
    output:
        tuple val(meta), path("${meta.alias}_hamro_rgi.tsv")
    script:
    """
    hamronize rgi --reference_database_version 6.0.3 --analysis_software_version 6.0.3 --input_file_name ${meta.alias} --output ${meta.alias}_hamro_rgi.tsv ${meta.alias}_rgi_results.txt
    """
}

process hamronize_all {
    label "hamronize"
    cpus 1
    memory "1 GB"
    input:
    ...
    output:
        tuple val(meta), path("${meta.alias}_hamronized.tsv")
    script:
    """
    hamronize summarize -o ${meta.alias}_hamronized.tsv -t tsv $reports
    """
}
...
workflow run_isolates {
   ...
   main:
        ...
        amrfinder_results = amrfinder(consensus)    // TODO: pass species
        rgi_results = rgi(consensus)
        // Serotyping
        serotype = serotyping(resfinder_input
            | filter { meta, fasta, species -> species == "salmonella" }
        )
        // hAMRonization - harmonize across the tools
        resfinder_hamronized = resfinder_hamronize(resfinder_results)
        amrfinder_hamronized = amrfinder_hamronize(amrfinder_results)
        rgi_hamronized = rgi_hamronize(rgi_results)
        hamronized_amr = hamronize_all(
            amrfinder_hamronized.join(
            rgi_hamronized).join(
            resfinder_hamronized)
            | map{ tup -> [tup.first(), tup.tail()] } // groupTuple not in Nextflow 23.04
        )
  
   emit:
      amr = resfinder_results
      mlst = mlst_results
      serotype = serotype
